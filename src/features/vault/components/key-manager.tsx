...
export function KeyManager() {
  ...
  const [activeTab, setActiveTab] = useState<"ssh-keys" | "host-keys">(
    "ssh-keys"
  )
  ...
  async function exportPrivate(key: SshKeyDto) {
    ...
    if (!master.trim()) {
      toast.error(
        "Password required",
        "Master password is required to export a private key."
      )
      return
    }
    try {
  ...
  const canImport =
    !!importPath || (!!importContent.trim() && !!importName.trim())
    ...
    <div className="flex h-full min-h-0 flex-col overflow-hidden">
      ...
      >
        <div className="shrink-0 border-b border-border px-5 py-2">
          <TabsList
            variant="line"
            className="h-9 w-fit gap-1 bg-transparent p-0"
          >
        ...
        >
          <div className="flex h-full min-h-0">
            ...
            <aside className="flex w-[340px] shrink-0 flex-col gap-0 overflow-y-auto border-r border-border bg-muted/15">
              ...
              <section className="flex flex-1 flex-col space-y-3 p-4">
                ...
                <p className="text-[11px] leading-relaxed text-muted-foreground">
                  ...
                  file (often no extension) — not{" "}
                  <code className="font-mono">*.pub</code>.
                </p>
                <label className="block space-y-1">
                  <span className="text-[11px] text-muted-foreground">
                    Name in vault
                ...
                {importPath && (
                  <div className="flex items-center gap-2 rounded-md border border-border bg-background px-2.5 py-2 text-[11px]">
                ...
                <Button
                  ...
                  onClick={() => {
                    if (!importPath && !importName.trim()) {
                      toast.error(
                        "Name required",
                        "Give the key a name before importing."
                      )
                      return
                    }
                    importKey.mutate()
            ...
            <main className="flex min-h-0 min-w-0 flex-1 flex-col">
              <div className="flex shrink-0 items-center justify-between border-b border-border px-5 py-2.5 text-[11px] font-semibold tracking-wider text-muted-foreground uppercase">
                <span>Workspace keys</span>
                <span className="font-mono tracking-normal normal-case opacity-70">
                  {keyCount} stored
                </span>
              </div>
              ...
              <div className="min-h-0 flex-1 overflow-y-auto p-4">
                ...
                {filteredKeys.length === 0 && (
                  <div className="flex h-full min-h-48 flex-col items-center justify-center rounded-xl border border-dashed border-border px-6 text-center text-muted-foreground">
                    <KeyRound className="mb-3 size-8 opacity-30" />
                    <p className="text-sm font-medium text-foreground/80">
                      No keys in this workspace
                    </p>
                    <p className="mt-1 max-w-sm text-xs">
                      Generate a new key on the left, or import an existing
                      OpenSSH private key.
                    </p>
                  </div>
                )}
                    ...
                    return (
                      <li
                        ...
                        className={cn(
                          ...
                          open && "shadow-sm ring-1 ring-primary/20"
                        )}
                      >
                          ...
                          <div className="min-w-0 flex-1">
                            {renaming ? (
                              ...
                              <button
                                ...
                                onClick={() =>
                                  setExpandedId(open ? null : k.id)
                                }
                              >
                                {k.name}
                              </button>
                            )}
                            <div className="mt-1.5 flex flex-wrap items-center gap-1.5 font-mono text-[11px] text-muted-foreground">
                              <span className="rounded bg-muted px-1.5 py-0.5 text-[10px] font-semibold tracking-wide uppercase">
                                {k.keyType}
                              </span>
                              <span
                                className="opacity-80"
                                title={k.fingerprintSha256}
                              >
                        ...
                        {open && (
                          <div className="border-t border-border bg-muted/25 px-4 py-3">
                            ...
                            <pre className="max-h-32 overflow-auto rounded-lg border border-border bg-background p-3 font-mono text-[11px] leading-relaxed break-all whitespace-pre-wrap">
                              {k.publicKey}
                            </pre>
