...
export function useMcpState() {
  ...
  const [activePairing, setActivePairing] = useState<McpPairingResult | null>(
    null
  )
  const [activePairingCode, setActivePairingCode] =
    useState<McpPairingCode | null>(null)
  ...
  const setClientMode = async (clientId: string, mode: ClientMode) => {
    try {
      ...
      setError(
        typeof e === "string"
          ? e
          : err?.message || "Failed to update client mode"
      )
    }
  }
  ...
  const setClientEnabled = async (clientId: string, enabled: boolean) => {
    try {
      ...
      const err = e as Error
      setError(
        typeof e === "string" ? e : err?.message || "Failed to toggle client"
      )
    }
  }
  ...
  const deleteClient = async (clientId: string) => {
    try {
      ...
      const err = e as Error
      setError(
        typeof e === "string" ? e : err?.message || "Failed to delete client"
      )
    }
  }
  ...
  const grantHost = async (clientId: string, hostId: string) => {
    try {
      await invoke("mcp_client_grant_host", {
        clientId,
        hostId,
        execAllowed: true,
        writeAllowed: true,
      ...
      const err = e as Error
      setError(
        typeof e === "string" ? e : err?.message || "Failed to grant host"
      )
    }
  }
  ...
  const revokeHost = async (clientId: string, hostId: string) => {
    try {
      ...
      const err = e as Error
      setError(
        typeof e === "string" ? e : err?.message || "Failed to revoke host"
      )
    }
  }
  ...
  useEffect(() => {
    ...
    const unlistenServerState = listen<McpServerState>(
      "mcp://server-state",
      (event) => {
