...
export function McpApprovalDialog() {
  ...
  const handleRespond = async (decision: ApprovalDecision) => {
    ...
    try {
      ...
      if (message.includes('already resolved') || message.includes('timed out')) {
        toast.error('Approval expired', 'This request was denied automatically or already resolved.');
      } else if (message.includes('no pending approval gate')) {
        toast.error('Approval unavailable', 'The MCP request may have timed out. Try again from the agent.');
      } else {
  ...
  const handleAllowOnce = () => {
    if (current.production && current.riskTier === 'dangerous' && !confirmProduction) {
  ...
  return (
    ...
    <Dialog open onOpenChange={() => undefined}>
      <DialogContent
        ...
        className={cn(
          'sm:max-w-lg gap-3',
          current.production && current.riskTier === 'dangerous' && 'border-destructive/50',
        )}
      >
        <DialogHeader>
          <div className="flex items-start justify-between gap-3">
            <div className="space-y-1">
              ...
              <DialogDescription className="text-xs">
                Requested by <span className="font-medium text-foreground">{current.clientName}</span>
              </DialogDescription>
            </div>
        ...
        <div className="grid grid-cols-3 gap-2 text-xs">
          ...
          <div className="rounded-xl border border-border/70 bg-muted/30 p-2">
            <span className="text-muted-foreground block mb-0.5">Host</span>
            <span className="font-mono text-xs">{current.hostLabel ?? '—'}</span>
            {current.production && (
          ...
          <div className="rounded-xl border border-border/70 bg-muted/30 p-2">
            <span className="text-muted-foreground block mb-0.5">Tool</span>
        ...
        {current.agentReason && (
          <div className="rounded-xl border border-border/70 bg-muted/20 p-3 text-xs">
            <span className="font-medium block mb-1">Agent reason</span>
            <p className="text-muted-foreground whitespace-pre-wrap">{current.agentReason}</p>
          </div>
        )}
        ...
        {current.commandText && (
          <div className="space-y-1.5">
            <div className="flex items-center justify-between">
              <span className="text-xs font-medium text-muted-foreground">Command (exact bytes)</span>
              <Button variant="ghost" size="sm" className="h-7 px-2 text-xs" onClick={copyCommand}>
                {copied ? <Check className="h-3.5 w-3.5" /> : <Copy className="h-3.5 w-3.5" />}
                Copy
              </Button>
            </div>
            <ScrollArea className="max-h-40 rounded-xl border border-border/70 bg-muted/20">
              <pre className="p-3 font-mono text-xs whitespace-pre select-all overflow-x-auto">
        ...
        {current.riskReasons.length > 0 && (
          <div className="rounded-xl border border-border/70 bg-muted/20 p-3 text-xs space-y-1">
            <span className="font-medium block">Risk classifier triggers</span>
            ...
            <p className="text-[11px] text-muted-foreground pt-1">
              Risk analysis is advisory. Approve only if you understand the command.
            </p>
          </div>
        )}

        <div className="rounded-xl border border-border/70 bg-muted/20 p-3 text-xs">
          <span className="font-medium block mb-1">Dry-run preview</span>
          {current.previewOutput ? (
            <pre className="font-mono text-xs whitespace-pre-wrap select-all">{current.previewOutput}</pre>
          ) : (
            <span className="text-muted-foreground">Preview unavailable</span>
          )}
        </div>

        <div className="flex items-center justify-between text-xs text-muted-foreground border-t border-border/70 pt-2">
          <span className="flex items-center gap-1.5">
            <Clock className="h-3.5 w-3.5" />
            Denied automatically in {timeLeftSecs}s — denial is the default outcome.
          </span>
        </div>
