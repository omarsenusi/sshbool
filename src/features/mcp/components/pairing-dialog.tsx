...
import { ShieldCheck, Copy, Check, AlertTriangle, KeyRound, Hash } from 'lucide-react';
...
function pairCurlExample(port: number, code: string, clientName: string): string {
...
export function PairingDialog({
  ...
  const copyCurl = () => {
    if (!pairingCode) return;
    void navigator.clipboard.writeText(pairCurlExample(port, pairingCode.code, clientName));
    setCopiedCurl(true);
      ...
      <DialogContent className="sm:max-w-lg gap-3">
        <DialogHeader>
          ...
          <DialogDescription className="text-xs">
            Create a token in-app, or generate a 6-digit code for HTTP pairing from another tool.
          </DialogDescription>
        </DialogHeader>
          ...
          >
            ...
            <TabsContent value="code" className="mt-3 space-y-3">
              ...
              <p className="text-[11px] text-muted-foreground">
                ...
                <code className="text-xs">POST /mcp/pair</code> — the response includes the access
                token.
              </p>
            </TabsContent>
          </Tabs>
        ...
        {showTokenResult && (
          <div className="space-y-3">
            <div className="flex items-start gap-2 rounded-xl border border-amber-500/30 bg-amber-500/5 p-3 text-xs text-amber-800 dark:text-amber-200">
              ...
              <span>
                This token is shown <strong>once</strong>. Copy it now — SSHBool stores only a hash.
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                ...
                <Button variant="ghost" size="sm" className="h-7 px-2 text-xs" onClick={copyToken}>
                  {copiedToken ? <Check className="h-3.5 w-3.5" /> : <Copy className="h-3.5 w-3.5" />}
                  {copiedToken ? 'Copied' : 'Copy token'}
                </Button>
              </div>
              <pre className="max-w-md w-full rounded-xl border border-border/70 bg-muted/20 p-2 text-[10px] font-mono break-all select-all">
                {pairing.accessToken}
              </pre>
            </div>
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                ...
                <Button variant="ghost" size="sm" className="h-7 px-2 text-xs" onClick={copyConfig}>
                  {copiedConfig ? <Check className="h-3.5 w-3.5" /> : <Copy className="h-3.5 w-3.5" />}
                  {copiedConfig ? 'Copied' : 'Copy config'}
                </Button>
              </div>
              <ScrollArea className="max-h-48 max-w-md w-full rounded-xl border border-border/70 bg-muted/20">
                <pre className="p-3 text-[10px] font-mono whitespace-pre select-all">
                  {pairing.cursorConfig}
                </pre>
              </ScrollArea>
              <p className="text-[11px] text-muted-foreground">
                Paste into Cursor → Settings → MCP (or{' '}
                <code className="text-xs">~/.cursor/mcp.json</code>), then restart Cursor.
              </p>
            </div>
          </div>
        )}
        ...
        {showCodeResult && (
          <div className="space-y-3">
            <div className="flex items-center justify-between rounded-xl border border-border/70 bg-muted/20 p-4">
              <div>
                <p className="text-[11px] text-muted-foreground mb-1">Pairing code</p>
                <p className="font-mono text-2xl font-semibold tracking-[0.35em]">
                  {pairingCode.code}
                </p>
              </div>
              <div className="text-right">
                <p className="text-[11px] text-muted-foreground mb-1">Expires in</p>
            ...
            <div className="space-y-1.5">
              <div className="flex items-center justify-between">
                ...
                <div className="flex gap-1">
                  ...
                  <Button variant="ghost" size="sm" className="h-7 px-2 text-xs" onClick={copyCurl}>
              ...
              <ScrollArea className="max-h-32 rounded-xl border border-border/70 bg-muted/20">
                <pre className="p-3 text-[10px] font-mono whitespace-pre-wrap select-all">
                  {pairCurlExample(port, pairingCode.code, clientName)}
                </pre>
              </ScrollArea>
              <p className="text-[11px] text-muted-foreground">
                The JSON response includes <code className="text-xs">access_token</code>. Use it as{' '}
                <code className="text-xs">Authorization: Bearer …</code> in your MCP client config.
        ...
        <DialogFooter className="gap-2 sm:justify-between">
          ...
          {!showTokenResult && !showCodeResult && method === 'code' && (
            <Button size="sm" className="h-8" disabled={codeLoading} onClick={onCreateCode}>
