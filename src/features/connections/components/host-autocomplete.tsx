...
export function HostAutocomplete({
  ...
  return (
    <div
      ref={rootRef}
      className={cn("relative w-full max-w-md space-y-2", className)}
    >
      <div className="relative">
        ...
        <Input
          ...
          disabled={disabled}
          placeholder={
            selectedHost ? `Selected: ${selectedHost.label}` : placeholder
          }
        ...
        {open && results.length > 0 && (
          <div className="absolute top-[calc(100%+4px)] z-50 w-full max-w-md rounded-xl border border-border/70 bg-popover p-1 shadow-md ring-1 ring-foreground/10">
            ...
            {hasMore && (
              <p className="border-t border-border/60 px-2 py-1.5 text-[10px] text-muted-foreground">
                Showing {HOST_AUTOCOMPLETE_MAX_RESULTS} matches — refine your
                search
              </p>
            )}
          </div>
        ...
        {open && query.trim() && results.length === 0 && (
          <div className="absolute top-[calc(100%+4px)] z-50 w-full max-w-md rounded-xl border border-border/70 bg-popover px-3 py-2 text-xs text-muted-foreground shadow-md">
