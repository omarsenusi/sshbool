...
const FEATURE_REGISTRY: FeatureItem[] = [
  ...
  },
  {
    ...
    title: "Cloud Sync",
    description:
      "Securely sync your hosts, sessions, and configurations across devices.",
    icon: RefreshCw,
  ...
  },
  {
    ...
    description:
      "Manage Docker containers, images, volumes, and logs natively.",
    icon: Layers,
...
export function LicenseSettings() {
  ...
  const unlockedFeatures = FEATURE_REGISTRY.filter(
    (f) =>
  ...
  function copyLicenseKey() {
    ...
    toast.success(
      "License Key Copied",
      "Device Machine ID copied to clipboard."
    )
    ...
    <div className="max-w-4xl space-y-6">
      ...
      <div>
        ...
        <p className="mt-0.5 text-xs text-muted-foreground">
          Manage your software license status, hardware binding, and active
          enterprise features.
        </p>
      </div>
        ...
        <div className="flex flex-col justify-between gap-4 md:flex-row md:items-center">
          <div className="space-y-1.5">
            <div className="flex items-center gap-2">
              ...
              <span className="rounded-md border border-border bg-muted px-2 py-0.5 font-mono text-[10px] font-medium text-muted-foreground uppercase">
                {tier} TIER
              </span>
            </div>
            <h3 className="text-sm font-semibold text-foreground">
              {isActivated
                ? `${tier.charAt(0).toUpperCase() + tier.slice(1)} License`
                : "Free Tier Workspace"}
            </h3>
          ...
          <div className="flex shrink-0 items-center gap-2">
            <div className="min-w-[120px] rounded-lg border border-border bg-muted/40 px-3.5 py-2 text-right">
      ...
      <div className="space-y-3.5 rounded-xl border border-border bg-card p-5 shadow-sm">
        ...
        <div className="flex items-center gap-2">
          <div className="relative flex-1">
            <Input
              readOnly
              value={licenseKey}
              className="h-9 border-border bg-muted/30 pr-10 font-mono text-xs tracking-wider text-foreground select-all focus-visible:ring-0"
            />
            <Cpu className="absolute top-2.5 right-3 size-4 text-muted-foreground/40" />
          </div>
        ...
        <p className="text-[11px] text-muted-foreground">
          This Machine Key is generated from your hardware UUID. It remains
          persistent whether you reboot, close the app, or switch OS.
        </p>
      </div>
          ...
          <div className="grid grid-cols-1 gap-3 sm:grid-cols-2 md:grid-cols-3">
            {unlockedFeatures.map((f) => {
              ...
              return (
                <div
                  key={f.key}
                  className="space-y-1.5 rounded-xl border border-border bg-card p-4 shadow-sm transition-colors hover:border-muted-foreground/20"
                >
