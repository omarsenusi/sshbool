...
function readRiskTier(payload: Record<string, unknown>): McpApprovalRequest['riskTier'] {
...
export function normalizeApproval(payload: Record<string, unknown>): McpApprovalRequest {
  ...
  return {
    ...
    clientName: String(payload.clientName ?? payload.client_name ?? 'Unknown client'),
    hostId: hostIdRaw != null && hostIdRaw !== '' ? String(hostIdRaw) : null,
    hostLabel: hostLabelRaw != null && hostLabelRaw !== '' ? String(hostLabelRaw) : null,
    ...
    riskReasons: Array.isArray(riskReasonsRaw)
      ...
      : [],
    commandText: commandRaw != null && commandRaw !== '' ? String(commandRaw) : null,
    agentReason:
      agentReasonRaw != null && agentReasonRaw !== '' ? String(agentReasonRaw) : null,
    previewOutput: previewRaw != null && previewRaw !== '' ? String(previewRaw) : null,
...
export const useMcpApprovalStore = create<McpApprovalStore>((set, get) => ({
  ...
  init: async () => {
    ...
    try {
      const pending = await invoke<McpApprovalRequest[]>('mcp_approvals_pending');
      set({
        queue: pending.map((row) => normalizeApproval(row as unknown as Record<string, unknown>)),
        initialized: true,
      });
    } catch (e) {
    ...
    if (!unlistenApproval) {
      const unlisten = await listen<Record<string, unknown>>('mcp://approval-request', (event) => {
...
export function selectCurrentApproval(state: McpApprovalStore): McpApprovalRequest | null {
